...
export class PaymentsService {
  ...
  async initialize(
    ...
    if (!userEmail) {
      throw new BadRequestException(
        'Valid user email is required to process payment',
      );
    }
  ...
  async handleWebhook(body: any, signature: string, rawBody?: Buffer) {
    ...
    if (event === 'charge.success' || event === 'charge.failed') {
      ...
      if (transactionRef) {
        ...
        try {
          ...
          console.log(
            `Webhook handled event ${event} for ref: ${transactionRef}`,
          );
        }
      }
    }
    ...
    if (event === 'refund.processed') {
      const transactionRef =
        eventData?.transaction_reference || eventData?.transaction?.reference;
    ...
    if (event === 'refund.failed') {
      const transactionRef =
        eventData?.transaction_reference || eventData?.transaction?.reference;
      console.error(
        `Paystack refund failed for transaction ref: ${transactionRef}. Reason: ${eventData?.reason || 'Unknown'}`,
      );
